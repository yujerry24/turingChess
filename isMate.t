...
function isMate (pieceArray : array 1 .. 8, 1 .. 8 of int, kingPos : array 1 .. 2, 1 .. 2 of int, checkTeam : int) : boolean
    var controlArray : array 1 .. 8, 1 .. 8 of int := createControlArray (pieceArray)
    var team : int
    var piece : int
    var pawnMoveDir : int
	...
	for xpos : -1 .. 1
		...
		if ypos + kingPos (checkTeam, 1) <= 8 and ypos + kingPos (checkTeam, 1) >= 1 and xpos + kingPos (checkTeam, 2) <= 8 and xpos + kingPos (checkTeam, 2) >= 1 then
			...
			if pieceArray (kingPos (checkTeam, 1) + ypos, kingPos (checkTeam, 2) + xpos) = 30 then
			    put "King can move to (", kingPos (checkTeam, 1) + ypos, ", ", kingPos (checkTeam, 2) + xpos, ")"
			    Input.Pause
			    result false
			end if
		    end if
	...
	for x : 1 .. 8
		...
		if not team = checkTeam then %Only enemy pieces keep the king in check
			...
			for xpos : 1 .. 7                 %Anything plus 8 will always be out of range
				...
				elsif (controlArray (y + xpos, x + xpos) mod 10 = team) or (controlArray (y + xpos, x + xpos) mod 10 = 3) then
				    blockPossible := true
				end if
			...
			for pos : y .. 8                 %Look vertically below
				...
				elsif (controlArray (pos, x) mod 10 = team) or (controlArray (pos, x) mod 10 = 3) then
				    blockPossible := true
				end if
				exit when not pieceArray (pos, x) = 30
			    end if
			...
			for pos : x .. 8                 %Look to the right
				...
				elsif (controlArray (y, pos) mod 10 = team) or (controlArray (y, pos) mod 10 = 3) then
				    blockPossible := true
				end if
				exit when not pieceArray (y, pos) = 30
			    end if
